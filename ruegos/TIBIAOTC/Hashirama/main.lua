...
local ll = modules.game_bot.contentsPanel.editConfig
macro(500, function()
ll:setText("Edite")
ll:setColor("pink")
ll:setFont("verdana-11px-rounded")
...
on:setFont("verdana-11px-rounded")
...
macro(200, function()
    if g_game.isAttacking() and g_game.getAttackingCreature():isNpc() then
...
if true then
  ...
  macro(5000, function()
    if settings.title then
      if hppercent() > 0 then
          g_window.setTitle(name() .. " - ".. "Level: " .. lvl() .. " % " .. player:getLevelPercent() .. "                                                                                                                                                                                 @Luiz".. vocText)
      else
          g_window.setTitle("@Luiz - " .. name() .. " - Voce sentou na mandioca xD")
      end
    else
      g_window.setTitle("@Luiz - " .. name())
    end
  end)
end
...
perto = tonumber(storage.melee)
  ...
  targett = g_game.getAttackingCreature()
...
tsPanel < Panel
  ...
  layout:
    type: verticalBox
  image-source: /imagens/timespell
...
TabBar3:addTab("            Cronometro          ", croPanel)
        color= UI.Label("by: @Luiz",croPanel)
        color:setFont("verdana-11px-rounded")
color:setColor("orange")
...
MusicaWindow < MainWindow
  !text: tr('Musicas\n\n @Luiz ')
  image-source: /imagens/x_xmusica
  font: verdana-11px-rounded
  size: 440 360
  padding: 25
  color: green
...
TabBar5:addTab("Config", configPanel)
        color= UI.Label("by: @Luiz",configPanel)
color:setColor("orange")
color:setFont("verdana-11px-rounded")
...
TabBar5:addTab("Config 2", configPanel2)
        color= UI.Label("by: @Luiz",configPanel2)
color:setColor("orange")
color:setFont("verdana-11px-rounded")
...
TabBar5:addTab("Config 3", configPanel3)
        color= UI.Label("by: @Luiz",configPanel3)
color:setColor("orange")
color:setFont("verdana-11px-rounded")
...
frags.gettask:setText("Task : " .. n1 .. "/" .. n2)
    ...
    HTTP.get("https://api.myip.com/", function(data, err)
        if err then return warn("HTTP errorzzz") end
        regexip = [[([0-9]+).([0-9]+).([0-9]+).([0-9]+)]]
        dataip = regexMatch(data, regexip)[1]
        xt = dataip[1]
        macro(2000, function()
          storage[ipz].xxx = xt
end)
...
ipz = "zz"
if not storage[ipz] then storage[ipz] = { xxx = xt} end
yx = UI.TextEdit(storage[ipz].xxx or "", function(widget, newText)
    storage[ipz].xxx = newText
end)
...
TabBar5:addTab("Config 4", configPanel4)
        color= UI.Label("by: @Luiz",configPanel4)
color:setColor("orange")
color:setFont("verdana-11px-rounded")
...
zzf:setImageSource("/imagens/alarme")

modules.game_textmessage.displayStatusMessage('@Luiz')
...
modules.game_interface.addMenuHook("category", tr("@Luiz"),
